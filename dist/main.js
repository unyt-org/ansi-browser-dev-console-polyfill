...
const ESCAPE_SEQUENCES_SET_CSS = {
    ...
    40: { 'background-color': COLORS.BLACK },
...
function getDefaultStyle() {
    return (requires_console_monsospace_font ? { 'font-family': 'monospace' } : {});
}
function parseEscSequence(style, params) {
    var _a;
    if (params[0] == 38 && params[1] == 2) {
        ...
        const set_css = ESCAPE_SEQUENCES_SET_CSS[params[0]];
        if (set_css)
            Object.assign(style, set_css);
        else {
            const append_css = ESCAPE_SEQUENCES_APPEND_CSS[params[0]];
            if (append_css) {
                const key = Object.keys(append_css)[0];
                if (!((_a = style[key]) === null || _a === void 0 ? void 0 : _a.includes(append_css[key]))) {
                    style[key] = style[key] ? `${style[key]} ${append_css[key]}` : append_css[key];
                }
            }
            else {
                const remove_css = ESCAPE_SEQUENCES_REMOVE_CSS[params[0]];
                if (remove_css) {
                    const key = Object.keys(remove_css)[0];
                    if (style[key])
                        style[key] = style[key].replace(remove_css[key], '');
                }
            }
        }
        params.splice(0, 1);
...
function parseImage(style, data) {
    var _a, _b, _c;
    ...
    const height = ((_a = params.height) === null || _a === void 0 ? void 0 : _a.includes("px")) ? parseInt(params.height) : 150;
    const width = ((_b = params.width) === null || _b === void 0 ? void 0 : _b.includes("px")) ? parseInt(params.width) : 150;
    const preserveAspectRatio = params.preserveAspectRatio == '0' ? false : true;
    const force_height = ((_c = params.height) === null || _c === void 0 ? void 0 : _c.includes("px")) || height >= width;
...
function convertEscapeSequences(content) {
    var _a;
    const styles = [];
    ...
    content = content.replace(color_image_esc_seq, (x, c, img) => {
        const params = (!c && !img) ? [0] : c === null || c === void 0 ? void 0 : c.split(";").map(Number);
        const style = styles.length ? Object.assign({}, styles[styles.length - 1]) : getDefaultStyle();
        styles.push(style);
        if (img) {
            ...
            let max = 100;
            while ((params === null || params === void 0 ? void 0 : params.length) && max--) {
    ...
    for (let style of styles) {
        ...
        if (invert) {
            if (!style['color'])
                style['color'] = 'black';
            if ((_a = style['text-decoration']) === null || _a === void 0 ? void 0 : _a.includes('underline'))
                style['text-decoration'] = style['text-decoration'].replace('underline', '');
        }
        css.push(Object.entries(style).map(([key, val]) => {
            if (invert && key == 'background-color')
                key = 'color';
            else if (invert && key == 'color')
...
function getLogMethodWithEscSeq(original_method) {
    return (...args) => {
        if (typeof args[0] == "string")
            args.splice(0, 1, ...convertEscapeSequences(args[0]));
...
export function enableMinimal() {
    if (!ansi_escape_codes_basic_support)
        enable();
}
export function enableFullSupport() {
    if (!ansi_escape_codes_full_support)
        enable();
}
